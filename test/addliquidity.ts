...
describe("AddLiquidity", function () {
  ...
  beforeEach(async function () {
    ...
    describe("AddLiquidity NEW PAIRTOKEN", function () {
      it("Should ADD WITH tokenA and TokenB PAIR2", async function () {


        const TokenXC = (await ethers.getContractFactory("Token")) as Token__factory;
        tokenXC = await TokenXC.deploy("X Coin", "XC");
        // const TokenBUSD = (await ethers.getContractFactory("Token")) as Token__factory;
        // tokenBUSD = await TokenBUSD.deploy("Binance USD", "BUSD");

        await tokenXC.connect(owner).mint(owner.address, ethers.utils.parseEther("600"));
        await tokenXC.connect(owner).approve(pancakeRouter.address, ethers.utils.parseEther("600"));
        await tokenBUSD.connect(owner).mint(owner.address, ethers.utils.parseEther("300"));
        await tokenBUSD.connect(owner).approve(pancakeRouter.address, ethers.utils.parseEther("300"));
        ...
        const getPairAdd = await contractFactory.getPair(tokenXC.address, tokenBUSD.address);
        expect(await contractFactory.allPairs(1)).to.equal(
          await contractFactory.getPair(tokenXC.address, tokenBUSD.address)
        );
      ...
      it("Should ADD MORE PAIR2", async function () {
